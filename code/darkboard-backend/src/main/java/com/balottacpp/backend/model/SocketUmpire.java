...
public class SocketUmpire extends Umpire implements Cloneable {
	
    static SocketIOClient playerSocket;
	 ...
	 * @param pl2
	 */
	public SocketUmpire(Player pl1, Player pl2, SocketIOClient playerSocket)
	{
	...
	public ExtendedPGNGame arbitrate(int initialPosition[][])
	{
		...
		while (true)
		{
			...
			if (m==null)
			{
				//if (verbose) playerSocket.sendEvent("read_message", boardState());
			}
			...
			if (isLegalMove(m,playerNumber))
			{
				...
				if (verbose) playerSocket.sendEvent("read_message", "Move "+m+" by Player "+playerNumber+" is legal.");
				...
				if (outcome==OUTCOME_CHECKMATE)
				{
					...
					if (verbose) playerSocket.sendEvent("read_message", "Player "+playerNumber+" wins!");
				...
				if (outcome==OUTCOME_ADJUDICATION)
				{
					...
					if (verbose) playerSocket.sendEvent("read_message", "Player "+(winner==getP1()?"0":"1")+" wins!");
				...
				if (outcome==OUTCOME_STALEMATE)
				{
					...
					if (verbose) playerSocket.sendEvent("read_message", "Stalemate!");
					getP1().receiveAftermath(transcript);
					getP2().receiveAftermath(transcript);
					return transcript;					
				}
				if (outcome==OUTCOME_FIFTY_MOVES)
				{
					...
					transcript.setResult("1/2-1/2");
					if (verbose) playerSocket.sendEvent("read_message", "Fifty Moves Draw!");
					getP1().receiveAftermath(transcript);
					getP2().receiveAftermath(transcript);
					return transcript;					
				}
				...
				if (outcome==OUTCOME_NO_MATERIAL)
				{
					...
					transcript.setResult("1/2-1/2");
					if (verbose) playerSocket.sendEvent("read_message", "No Material Draw!");
					getP1().receiveAftermath(transcript);
					getP2().receiveAftermath(transcript);
					return transcript;					
				}
				...
				opponent.communicateUmpireMessage(cx,cy,pawnTries,chk,chk2);
			...
			{
				//illegal move.
				if (verbose) playerSocket.sendEvent("read_message", "Move "+m+" by Player "+playerNumber+" is NOT legal (reason: "+
					illegalMoveReason+").");
	...
	public boolean isLegalMove(Move m, int player)
	{
		...
		if (pieceType!=Chessboard.KNIGHT && pieceType!=Chessboard.KING)
		{
			...
			{
				//playerSocket.sendEvent("read_message", "Checking "+x+" "+y+" found "+board[x][y]);
	...
	{		
		...
		//playerSocket.sendEvent("read_message", "Full move string is "+m.toString());
			...
			for (int j=0; j<8; j++)
			{
				if (k!=movx || j!=movy && umpire2ChessboardPieceCode(getBoard(k,j))==m.piece)
				{
					...
					if (legal || (!legal && !omniscient && (illegalMoveReason>=10)))
					{
						...
						//if (verbose) playerSocket.sendEvent("read_message", "Move "+movx+","+movy+" ambiguity with "+k+","+j);
						//if (verbose) playerSocket.sendEvent("read_message", this.boardState());
	...
	{
		//playerSocket.sendEvent("read_message", "Move String is " + s);
		Move m = new Move();
		...
		//playerSocket.sendEvent("read_message", this.boardState());
		
		return m;
	}

    public Move parseString(String s, boolean white) {
        Move m = new Move();
	...
	private void fillStartingSquare(Move m,boolean white)
	{
		...
		if (m.toX < 0 || m.toY < 0)
		{
			playerSocket.sendEvent("read_message", "MOVE FORMAT ERROR");
			return;
		}
		...
		for (int k=0; k<8; k++)
			for (int j=0; j<8; j++)
			{
				...
				{
					m.fromX = (byte)k; m.fromY = (byte)j;
					//playerSocket.sendEvent("read_message", "Checking validity of " + m);
	...
	public static int[][] boardLayoutFromFEN(String fen)
	{
		...
		for (int k=0; k<fen.length(); k++)
		{
			...
			switch (c)
			{
				...
				if (rank==0)
				{
					playerSocket.sendEvent("read_message", "FEN error, too many ranks.");
					return null;
				} else
				if (file!=8)
				{
					playerSocket.sendEvent("read_message", "FEN error, incorrect rank width.");
					return null;
				}
				else
				{
		...
		if (rank!=0 || file!=8)
		{
			playerSocket.sendEvent("read_message", "FEN error, unexpected end of string.");
			return null;
		}
	...
	{
		playerSocket.sendEvent("chessboard_changed", this.toFen());
	}
	...
	public boolean areMetapositionsCompatibleWithUmpireMessage(Metaposition w, Metaposition b, boolean white, int cx, int cy, /*int ctype,*/ int c1, int c2, int pawnTries) throws IncompatibleMetapositionException
	{
		if (!w.isWhite())
		{
		...
		for (int k=0; k<8; k++)
			for (int j=0; j<8; j++)
			{
				...
				if (blackOnSquare && whiteOnSquare && (k!=cx || j!=cy)) throw new IncompatibleMetapositionException(w, b);
				if (cx!=-1 && k==cx && j==cy)
				{
					if (!blackOnSquare || !whiteOnSquare) throw new IncompatibleMetapositionException(w, b);
					if (!white) continue;
	...
	public boolean areMetapositionsCompatibleWithIllegalMove(Metaposition w, Metaposition b, boolean white, Move m) throws IncompatibleMetapositionException
	{
		if (!w.isWhite())
		{
			Metaposition mp = w; w = b; b = mp;
		...
		for (int k=0; k<8; k++)
			for (int j=0; j<8; j++)
			{
				...
				pc = b.getFriendlyPiece(k, j);
				if (pc!=Chessboard.EMPTY && board[k][j]!=LocalUmpire.EMPTY) throw new IncompatibleMetapositionException(w,b);
				switch (pc)
				{
				case Chessboard.PAWN: board[k][j] = BP; break;
				case Chessboard.KNIGHT: board[k][j] = BN; break;
